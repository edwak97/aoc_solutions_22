...
def getPathNumber(steps:int = 1, nodelist = [entrance]):
		...
		return getPathNumber(steps + 1, nodelist = newnodelist) if newnodelist != [] else 0
#print(getPathNumber())
entrance.score = None
listtocheck = [elem for line in squares for elem in line if elem.height == 0]
resultsarray = []
for node in listtocheck:
	node.score = 0
	res = getPathNumber(nodelist = [node])
	...
	for line in squares:
		for elem in line:
			elem.score = None
resultsarray = [a for a in resultsarray if a > 0]
resultsarray = sorted(resultsarray)
