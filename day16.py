...
def getCombinationSingle(previousValves, released, T, nextNames, includeBad = False):
	...
	for i in range(len(nextNames)):
		...
		if includeBad:
			yield (_released, _previousValves)
		combination = getCombinationSingle(_previousValves, _released, _T, _nextNames, includeBad)
		while True:
			try:
...
baseCombination = getCombinationSingle(beginValve, 0, T, valveNames, True)

#print(getMaxForSingle(beginValve, 0, 30, valveNames)) #This one is correct # Part 1
